...
export async function createSale(input: unknown) { const id = await userId(); const data = z.object({ status: z.enum(["lead", "proposal", "won", "lost"]).default("lead"), amount: z.number().int().nonnegative().optional() }).parse(input); await db.insert(sales).values({ id: randomUUID(), userId: id, ...data }); revalidatePath('/studio/sales'); }
export async function createDocument(input: unknown) { const id = await userId(); const data = z.object({ title: z.string().trim().min(1).max(160), kind: z.string().trim().min(1).max(60) }).parse(input); await db.insert(documents).values({ id: randomUUID(), userId: id, ...data }); revalidatePath('/studio/documents'); }
