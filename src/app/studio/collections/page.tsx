...
export default async function CollectionsPage() {
  ...
  return <div className="flex flex-col gap-8"><StudioPageHeader eyebrow="Organization" title="Collections & places" description="Give your catalogue structure without forcing your practice into a template." action={<StudioButton><Plus data-icon="inline-start" />Create collection</StudioButton>} />{collections.length > 0 && <div className="grid gap-4 md:grid-cols-3">{collections.map((collection) => <article key={collection.id} className="studio-card flex min-h-40 flex-col p-6"><p className="studio-eyebrow">Collection</p><h2 className="mt-auto text-xl text-studio-ink">{collection.name}</h2><p className="mt-2 text-sm leading-6 text-studio-muted">{collection.description ?? "No description yet"}</p></article>)}</div>}<div className="grid gap-4 md:grid-cols-3">{modules.map(({ title, description, icon: Icon }) => <article key={title} className="studio-card flex min-h-56 flex-col p-6"><Icon className="text-studio-accent" aria-hidden /><h2 className="mt-auto text-xl text-studio-ink">{title}</h2><p className="mt-2 text-sm leading-6 text-studio-muted">{description}</p><button className="mt-5 flex items-center gap-2 text-sm font-medium text-studio-ink">Open module <ArrowUpRight aria-hidden /></button></article>)}</div></div>;
}
